...
slab = SlabAnalysis(name="{sysname:s}", input_path="{path:s}",
                    output_path="data", ref_name="{args.name_1:s}", ref_chains = (0, 99),
                    client_names = ['{args.name_2:s}'], client_chain_list = [(100,199)],
                    verbose=True)
...
# calculate contact map between name_1 and name_2
chainid_dict = dict({args.name_1:s} = (0,99), {args.name_2:s} = (100,199))
